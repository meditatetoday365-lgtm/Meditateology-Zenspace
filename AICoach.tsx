...
const AICoach: React.FC = () => {
  ...
  const playAudio = async () => {
    ...
    if (audioData) {
      ...
      // Critical for mobile/chrome stability: Resume context on user click
        ...
        <div className="glass rounded-3xl p-8 shadow-xl relative overflow-hidden">
          {isPlaying && (
            <div className="absolute inset-0 bg-brand-primary/5 pointer-events-none overflow-hidden">
              <div className="pulse-ring bg-brand-primary/10"></div>
            </div>
          )}
          ...
          {!script ? (
            ...
            <div className="animate-fade-in relative z-10">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                   <button 
                    ...
                    className={`flex-1 md:flex-none px-6 py-2 rounded-full text-sm font-bold flex items-center justify-center gap-2 transition-all ${isPlaying ? 'bg-red-500 text-white' : 'bg-brand-primary text-white hover:bg-brand-dark'}`}
                   >
                     {isAudioLoading ? (
                       ...
                       <><div className="w-2 h-2 bg-white rounded-full animate-pulse"></div> Stop Master</>
                     ) : (
                   ...
                   <button 
                    ...
                    className="px-4 py-2 text-stone-400 hover:text-stone-600 text-xs font-bold uppercase tracking-widest"
                   >
                    New
                   </button>
                </div>
              ...
              <div className="bg-brand-light rounded-xl p-6 text-stone-700 leading-relaxed whitespace-pre-wrap max-h-[400px] overflow-y-auto font-serif relative">
                {script}
              </div>
