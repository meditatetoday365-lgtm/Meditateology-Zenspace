        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
              // Reset local transcription or keep it? Let's clear after a bit
  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed bottom-8 right-8 z-[60] w-16 h-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 transform hover:scale-110 active:scale-95 ${isConnected ? 'bg-red-500' : 'bg-teal-700'}`}
      >
        {isConnected ? (
      ...
      {/* Voice UI Overlay */}
      {isOpen && (
        <div className="fixed inset-0 z-[55] flex items-center justify-center p-4 bg-stone-900/90 backdrop-blur-md animate-fade-in">
          <div className="text-center max-w-xl w-full">
            <div className="relative w-48 h-48 mx-auto mb-12">
              <div className={`absolute inset-0 rounded-full bg-teal-500/20 transition-all duration-500 ${isConnected ? 'scale-100' : 'scale-0'}`}></div>
              {isModelSpeaking && (
                <>
              ...
              <div className="relative w-full h-full rounded-full border border-teal-500/30 flex items-center justify-center bg-stone-800">
                 <div className="w-4 h-4 rounded-full bg-teal-500 animate-pulse"></div>
              </div>
            </div>
            
            <h2 className="text-2xl font-serif italic text-stone-200 mb-4">
