...
export const generateSpeech = async (text: string) => {
  ...
  try {
    // We send a bit more text for longer meditations, Gemini 2.5 Flash TTS handles up to ~2k chars well.
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [{ 
        parts: [{ 
          text: `Read this meditation script as a wise Zen Master. Speak very slowly, with long pauses between sentences for the listener to breathe. Voice: calm, warm, and grounding. Script: ${cleanText}` 
        }] 
      }],
      config: {
